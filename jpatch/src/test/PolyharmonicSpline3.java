...
public class PolyharmonicSpline3 {
	...
	PolyharmonicSpline3() {
		...
		for (int i = 0; i < num; i++) {
			...
			c[i][1] = Math.random();
			c[i][2] = Math.random();
			y[i] = 0.25 + 0.75 * Math.random();
			double dx = (c[i][0] - 0.5) * 1.414;
			double dy = (c[i][1] - 0.5) * 1.414;
			double dz = (c[i][2] - 0.5) * 1.414;
//			y[i] = 0.75 - Math.max(0, 0.5 * (dx * dx + dy * dy + dz * dz));
//			y[i] = (c[i][0] + c[i][1]) * 0.5;
		}
////		c[0][0] = 0.5;
////		c[0][1] = 0.5;
...
//		v = b[5];
	...
	private void draw(BufferedImage image, double z) {
