...
//		float epsilon = 3f / MainFrame.getInstance().getJPatchScreen().getActiveViewport().getViewDefinition().getMatrix().getScale();
	 ...
	 * Lathe Editor Dialog
	 */
	private class SegmentsDialog extends JDialog implements ActionListener, ChangeListener {
		private static final long serialVersionUID = 1L;
		private boolean okPressed = false;

		public SegmentsDialog(Frame owner) {
		...
		public void stateChanged(ChangeEvent evt) {
			if (evt.getSource() instanceof JSlider) {
				...
				if (evt.getSource() == formSlider) {
					...
					rotateLabel.setText("Rotation: " + val + "°");
					iRotation = val;
				}
		...
		private void initComponents() {
			...
			rotateLabel.setText("Rotation: " + iRotation + "°");
			gridBagConstraints = new java.awt.GridBagConstraints();
			gridBagConstraints.gridx = 2;
			gridBagConstraints.gridy = 4;
			gridBagConstraints.gridwidth = 2;
