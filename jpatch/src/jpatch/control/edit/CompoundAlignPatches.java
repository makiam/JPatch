...
public class CompoundAlignPatches extends JPatchCompoundEdit implements JPatchRootEdit {
	...
	**/
	private int comparePatches(Patch a, Patch b) {
		OLDControlPoint[] acpA = a.getControlPoints();
		int na = acpA.length;
		OLDControlPoint[] acpB = b.getControlPoints();
		int nb = acpB.length;
		for (int i = 0; i < acpA.length; i += 2) {
			for (int j = 0; j < acpB.length; j+= 2) {
				...
				/* treat hooks specially */
					...
					OLDControlPoint cpA = acpA[i].getParentHook();
					OLDControlPoint cpB = acpA[i].getEnd().getParentHook();
					if (cpA == acpB[j] && cpB == acpB[j + 1]) return -1;
					else if (cpA == acpB[j + 1] && cpB == acpB[j]) return 1;
				} else if (acpA[i + 1].isStartHook()) {
					OLDControlPoint cpA = acpA[i + 1].getParentHook();
					OLDControlPoint cpB = acpA[i + 1].getEnd().getParentHook();
					if (cpA == acpB[j] && cpB == acpB[j + 1]) return 1;
					else if (cpA == acpB[j + 1] && cpB == acpB[j]) return -1;
				} else if (acpB[j].isStartHook()) {
					OLDControlPoint cpA = acpB[j].getParentHook();
					OLDControlPoint cpB = acpB[j].getEnd().getParentHook();
					if (cpA == acpA[i] && cpB == acpA[i + 1]) return -1;
					else if (cpA == acpA[i + 1] && cpB == acpA[i]) return 1;
				} else if (acpB[j + 1].isStartHook()) {
					OLDControlPoint cpA = acpB[j + 1].getParentHook();
					OLDControlPoint cpB = acpB[j + 1].getEnd().getParentHook();
					if (cpA == acpA[i] && cpB == acpA[i + 1]) return 1;
					else if (cpA == acpA[i + 1] && cpB == acpA[i]) return -1;
				}
			}
