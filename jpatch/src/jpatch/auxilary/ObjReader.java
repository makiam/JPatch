...
public class ObjReader {
	...
	public AliasWavefrontObj readObj(String filename) {
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (token[0].equals("mtllib")) {
					...
					for (int i = 0; i < n; i++) {
						...
						ni[i] = Integer.parseInt(field[1]) - 1;
					}
					faces.add(vi);
					faces.add(ni);
					F++;
			...
			System.out.println(F + " faces, " + V + " vertices");
	...
	private Map readMat(File file) throws IOException, NumberFormatException {
