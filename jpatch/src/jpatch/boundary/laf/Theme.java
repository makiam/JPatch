/*
 * $Id: Theme.java,v 1.2 2005/08/23 20:16:35 sascha_l Exp $
 *
...
public class Theme {
	...
	public static void paintButtonBackground(AbstractButton button, Graphics2D g2) {
		...
		if (model.isRollover()) {
			if (model.isPressed()) {
				g2.setPaint(new GradientPaint(0, 0, button.getBackground(), 0, height - 2, Color.WHITE));
				g2.fill(new RoundRectangle2D.Float(1, 1, width - 2, height - 2, 7, 7));
			} else {
				g2.setPaint(new GradientPaint(0, 0, Color.WHITE, 0, height - 2, button.getBackground()));
				g2.fill(new RoundRectangle2D.Float(1, 1, width - 2, height - 2, 7, 7));
			}
