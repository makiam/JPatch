...
public class ColorPreferences extends JDialog implements ActionListener, WindowListener {
	...
	private Color[] colors = JPatchSettings.getInstance().getColors();
	private boolean bChanged = false;
	
	public ColorPreferences(Frame owner) {
		super(owner,"Color settings",true);
		
		setColors(JPatchSettings.getInstance().getColors());
	...
	private void defaults() {
		...
		if (option == JOptionPane.YES_OPTION) {
			setColors(JPatchSettings.getInstance().getDefaultColors());
			JPatchSettings.getInstance().setColors(getColors());
			setButtonColors();
	...
	private void cancel() {
		if (!bChanged) {
			...
			if (option == JOptionPane.YES_OPTION) {
				JPatchSettings.getInstance().setColors(colors);
				dispose();
				MainFrame.getInstance().getJPatchScreen().update_all();
			}
		}
